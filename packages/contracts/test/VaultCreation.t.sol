...
contract VaultCreationTest is Test {
    ...
    function _loadDeployedInfrastructure() internal {
        ...
        implementation = PortfolioRebalancer(vm.parseJsonAddress(addressBookJson, ".portfolioRebalancer.implementation"));
        ...
        for (uint256 i = 0; i < tokenAddresses.length; i++) {
            for (uint256 j = 0; j < tokenAddresses.length; j++) {
                if (i != j) {
                    vm.mockCall(
                        uniswapV3Factory,
                        abi.encodeWithSignature("getPool(address,address,uint24)", tokenAddresses[i], tokenAddresses[j], 3000),
        ...
        try vm.envString("PRIVATE_KEY") returns (string memory privateKeyStr) {
            ...
            try IERC20(linkToken).balanceOf(deployer) returns (uint256 deployerBalance) {
                ...
                if (deployerBalance >= requiredLINK) {
                    ...
                    if (success) {
                        console.log("Successfully funded treasury with", requiredLINK / 1e18, "LINK from deployer account");
                    } else {
